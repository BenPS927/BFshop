import { totalRevenue, totalSales } from "../../analytics/totals/totals"
...
import { AnalysisRequest, Orders } from "../../types/analysisMachine"
...
export async function analysisMachine(request: AnalysisRequest) {
    ...
    const results: { revenue?: number, sales?: number } = {};

    console.log(results)

    if (request.metrics.includes("revenue")) {
        results.revenue = totalRevenue(orders)
    } 
